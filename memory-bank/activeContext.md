...
- **Ctrl+Q detach latency eliminated** (`app/app.go` + `session/tmux/tmux_unix.go` + `session/tmux/tmux.go`): three root causes fixed — (1) `m.instanceChanged()` removed from post-detach callback (was blocking Bubbletea render with synchronous `tmux capture-pane -J` subprocess, ~10–100ms every detach); (2) `monitorWindowSize` goroutines removed from `t.wg` (was causing `Detach()`'s `wg.Wait()` to stall ~50ms probabilistically if 2s periodic poll was mid-`tmux resize-window`); (3) **root cause**: `makeNonBlockingFile()` added to `tmux_unix.go` and called in `Attach()` — PTY master fd from `creack/pty` is in blocking mode, so Go's `os.File.Close()` uses `runtime_Semacquire` to wait for the `io.Copy` goroutine's blocked `Read()` before closing the fd; when the agent is idle that `Read` never returns, causing an indefinite deadlock. Fix: dup the fd, `SetNonblock`, wrap with `os.NewFile` so Go registers it with the netpoller; `Close()` then calls `pd.evict()` which immediately unparks the blocked goroutine. Preview refreshes via `previewTickMsg` 100ms loop instead.
- **Ctrl+Q + terminal state fixes** (`session/tmux/tmux.go`): (1) stdin goroutine now scans full buffer for `0x11` instead of requiring `nr == 1` — pi/opencode flood stdin with mouse event bytes so Ctrl+Q arrived bundled; (2) `term.GetState` saved before PTY creation in `Attach()`, `term.Restore` + explicit ANSI disable sequences sent in `Detach()` to fix "extended keys are on" terminal leakage from pi/opencode; (3) `sizeMu sync.Mutex` added protecting `lastCols`/`lastRows`, reset to 0 in `Detach()` to fix resize flicker
- **Notification auto-ack removed** (`app.go`): auto-acknowledge that fired when agent went Ready while user was in that workspace has been removed — `ReadyAcknowledged` is now set only on Enter (attach). Test updated to match.
